  ...
  const CosmosTracker = {
    ...
    checkAndSendDelayedExitEvent: function () {
      ...
      if (timeSinceLastActivity >= sessionTimeoutSeconds) {
        ...
        console.log(
          "[CosMos] 📤 Sending delayed exit event for expired session:",
        ...
        console.log(
          "[CosMos] ✅ Delayed exit event sent for:",
    ...
    setupSessionTracking: function (urlParams) {
      ...
      if (isStoredSessionValid) {
        ...
        console.log(
          "[CosMos] 🔄 Continuing existing session:",
      ...
      if (isNewSession) {
        ...
        if (storedUTMData) {
          console.log(
            "[CosMos] 🔒 Using locked session UTMs (new UTMs ignored)"
          );
        }
      }
      ...
      if (flowData && flowData.session_id === this.sessionId) {
        ...
        console.log(
          "[CosMos] 📊 Restored session page count:",
    ...
    setupPageSequenceTracking: function () {
      ...
      if (isSequenceValid) {
        ...
        console.log(
          "[CosMos] 📈 Continuing page sequence:",
          this.pageSequence,
          "(from localStorage)"
        );
      } else {
      ...
      window.addEventListener("beforeunload", function (e) {
        ...
        if (storedExitCandidate) {
          try {
            ...
            console.log(
              "[CosMos] 💾 Exit candidate updated with time_on_page:",
              timeOnPage + "s"
            );
          } catch (err) {
